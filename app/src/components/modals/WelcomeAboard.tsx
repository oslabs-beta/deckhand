...
const Modal: React.FC = () => {
  ...
  const closeModal = () => {
    ...
    <div key="p0">
      ...
      <p>Ahoy! And welcome to the exciting world of Kubernetes. I'm your Deckhand.</p>
      <p>I'll help you deploy a Kubernetes cluster with no code. Fully automated. Open source.</p>
    </div>,
    <div key="p1">
      ...
      <p>Drag a Cluster element on the screen and select the resources (instance type and number of nodes) you’d like to provide for your application.</p>
    ...
    <div key="p2">
      ...
      <p>Next, drag over a Pod and connect it to your cluster. Here, you can choose between pulling in a Docker image or a GitHub repository.</p>
      ...
      <p>Do some parts of your application require particularly high availability and reliability? Simply adjust the number of replicas using the up and down arrows on the pod.</p>
    </div>,
      <div key="p3">
      ...
      <p>Drag over an Ingress element and connect it to a pod to define the entry point to your application. Deckhand will automatically scan the connected pod to find the correct ports upon deployment.</p>
      <h3 className="header"><Icon className="header-icon"  path={mdiImport} style={{ color: "green" }} size={1.25} />Variables Element</h3>
      <p>Have any environmental variables? Drag over a Variables element, connect it to the relevant pods, and Deckhand will scan the repository for the required variables. Simply enter those values where prompted, and Deckhand will encode and configure the ConfigMap and Secrets upon deployment.</p>
      <h3 className="header"><Icon className="header-icon"  path={mdiDatabase} style={{ color: "violet" }} size={1.25} />Volume Element</h3>
      <p>Need persistent storage? Drag over a Volume element, connect it to a pod, and enter the mount path. Deckhand will provision the necessary persistent volume claims, storage classes, and volumes upon deployment.</p>
    </div>,
    <div key="p4">
      <h1 className="header"><img className="header-icon"  alt="logo" src={logo} height="40" />Deployment</h1>
      <p>When you’re ready, just hit deploy. Deckhand will update with the URL for your running, load-balanced application.</p>
